...
class ConsumableService {
  ...
  Future<PagedResult<Consumable>> getAll({
    ...
    final response = await _dio.get('/api/assets/consumables', queryParameters: params);
  ...
  Future<List<Consumable>> getLowStock() async {
    ...
    return (response.data as List).map((e) => Consumable.fromJson(e as Map<String, dynamic>)).toList();
  }

  Future<List<StockMovement>> getMovementHistory(String id) async {
    final response = await _dio.get('/api/assets/consumables/movement-history/$id');
    return (response.data as List).map((e) => StockMovement.fromJson(e as Map<String, dynamic>)).toList();
  }

  Future<StockMovement> stockIn(String id, {required int quantity, String? reason}) async {
  ...
  Future<StockMovement> stockOut(String id, {required int quantity, String? reason}) async {
    final response = await _dio.post(
  ...
  Future<Consumable> update(String id, Map<String, dynamic> data) async {
    final response = await _dio.patch('/api/assets/consumables/$id', data: data);
    return Consumable.fromJson(response.data as Map<String, dynamic>);
  }
